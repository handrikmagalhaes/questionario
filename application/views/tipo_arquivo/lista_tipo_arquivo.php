...
$registros_por_pagina = 10;
			    ...
			    <?php if($_SESSION['inserir_tipo_arquivo']){ ?>
				  <a href="{url_base}tipo_arquivo/cadastro" class="btn btn-primary float-right" id="adicionar">Adicionar Novo</a>
				<?php } ?>
              </div>
              <!-- /.card-header -->
              <div class="card-body p-0" id="lista-tipos-arquivo">
                <table class="table table-striped">
                  <thead>                  
                    <tr>
                      <th onclick="ordena('TITULO_TIPO_ARQUIVO')" onmouseover="this.style.cursor='pointer';">Título</th>
                      <th width="150px">Ações</th>
                    </tr>
                  </thead>
                  <tbody>
                    <?php foreach ($dados['tipos_arquivo'] as $tipo_arquivo): ?>
                      <tr>
                        <td <?php if ($_SESSION['visualizar_tipo_arquivo']): ?>onclick="abreModal(<?php echo $tipo_arquivo->ID_TIPO_ARQUIVO; ?>)"<?php endif; ?> onmouseover="this.style.cursor='pointer';"><?php echo $tipo_arquivo->TITULO_TIPO_ARQUIVO; ?></td>
                        <td>
                          <?php if ($_SESSION['editar_tipo_arquivo']): ?> <!-- VERIFICAR PERMISSÃO -->
                            <form method="post" action="/tipo_arquivo/edicao" class="btn btn-sm border-0 p-0 m-0">
                              <input type="hidden" name="id" value="<?php echo $tipo_arquivo->ID_TIPO_ARQUIVO; ?>">
                          ...
                          <?php endif; ?>
                          <?php if ($_SESSION['excluir_tipo_arquivo']): ?> <!-- VERIFICAR PERMISSÃO -->
                            <button data-id="<?php echo $tipo_arquivo->ID_TIPO_ARQUIVO; ?>" class="btn btn-sm btn-outline-danger btn-excluir border-0 pt-0 pb-0" data-toggle="tooltip" data-placement="top" title="Excluir" data-original-title="Excluir"><i class="fas fa-trash"></i></button>
                          <?php endif; ?>
                        </td>
                      </tr>
                    <?php endforeach; ?>
  ...
  <div class="modal-dialog modal-dialog-scrollable">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="modalLabel">Dados do Tipo de Arquivo</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body" id="modal-dados">
        ...
        <p class="font-weight-normal" id="descricaoTipoArquivo"></p>
      </div>
      <div class="modal-footer">
      </div>
    </div>
