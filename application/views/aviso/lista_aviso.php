          ...
          <div class="col-md-12">
            <div class="card">
              <div class="card-header">
			    ...
			    <?php if($_SESSION['inserir_aviso']){ ?>
                	<a href="{url_base}aviso/cadastro" class="btn btn-primary float-right" id="adicionar">Adicionar Novo</a>
				<?php } ?>
              </div>
              <!-- /.card-header -->
              <div class="card-body p-0" id="lista-avisos">
                <table class="table table-striped">
                  ...
                  <tbody>
					...
					foreach ($dados['avisos'] as $aviso) {
					  if($aviso->ID_DEPARTAMENTO == null || $aviso->ID_DEPARTAMENTO == $_SESSION['departamento'] || $_SESSION['editar_aviso']){ ?>
                      <tr>
                        <td><?php echo $aviso->TITULO_AVISO; ?></td>
                        <td>
                          <?php if ($_SESSION['visualizar_aviso']): ?> <!-- VERIFICAR PERMISSÃO -->
                            <form method="post" action="../../../visualiza" class="btn btn-sm border-0 p-0 m-0">
                              <input type="hidden" name="id" value="<?php echo $aviso->ID_AVISO; ?>">
                          ...
                          <?php endif; ?>
                          <?php if ($_SESSION['editar_aviso']): ?> <!-- VERIFICAR PERMISSÃO -->
                          ...
                          <?php endif; ?>
                          <?php if ($_SESSION['excluir_aviso']): ?> <!-- VERIFICAR PERMISSÃO -->
                            <button data-id="<?php echo $aviso->ID_AVISO; ?>" class="btn btn-sm btn-outline-danger btn-excluir border-0 pt-0 pb-0" data-toggle="tooltip" data-placement="top" title="Excluir" data-original-title="Excluir"><i class="fas fa-trash"></i></button>
                          <?php endif; ?>
                        </td>
                      </tr>
					<?php
