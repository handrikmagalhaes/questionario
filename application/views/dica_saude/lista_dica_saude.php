...
$registros_por_pagina = 10;
			    ...
			    <?php if($_SESSION['inserir_dica_saude']){ ?>
				  <a href="{url_base}dica_saude/cadastro" class="btn btn-primary float-right" id="adicionar">Adicionar Novo</a>
 				<?php } ?>
              </div>
              <!-- /.card-header -->
              <div class="card-body p-0" id="lista-dicas-saude">
                <table class="table table-striped">
                  <thead>                  
                    <tr>
                      <th onclick="ordena('TITULO_DICA_SAUDE')" onmouseover="this.style.cursor='pointer';">Título</th>
                      <th width="150px">Ações</th>
                    </tr>
                  </thead>
                  <tbody>
                    <?php foreach ($dados['dicas_saude'] as $dica_saude): ?>
                      <tr>
                        <td <?php if ($_SESSION['visualizar_dica_saude']): ?>onclick="abreModal(<?php echo $dica_saude->ID_DICA_SAUDE; ?>)"<?php endif; ?> onmouseover="this.style.cursor='pointer';"><?php echo $dica_saude->TITULO_DICA_SAUDE; ?></td>
                          <td>
                          <?php //if ($_SESSION['visualizar_dica_saude']): ?> <!-- VERIFICAR PERMISSÃO -->
                            <!--<form method="post" action="../../../visualiza" class="btn btn-sm border-0 p-0 m-0">
                              <input type="hidden" name="id" value="<?php echo $dica_saude->ID_DICA_SAUDE; ?>">
                          ...
                          <?php //endif; ?>
                          <?php if ($_SESSION['editar_dica_saude']): ?> <!-- VERIFICAR PERMISSÃO -->
                            <form method="post" action="/dica_saude/edicao" class="btn btn-sm border-0 p-0 m-0">
                              <input type="hidden" name="id" value="<?php echo $dica_saude->ID_DICA_SAUDE; ?>">
                          ...
                          <?php endif; ?>
                          <?php if ($_SESSION['excluir_dica_saude']): ?> <!-- VERIFICAR PERMISSÃO -->
                            <button data-id="<?php echo $dica_saude->ID_DICA_SAUDE; ?>" class="btn btn-sm btn-outline-danger btn-excluir border-0 pt-0 pb-0" data-toggle="tooltip" data-placement="top" title="Excluir" data-original-title="Excluir"><i class="fas fa-trash"></i></button>
                          <?php endif; ?>
                        </td>
                      </tr>
                    <?php endforeach; ?>
  ...
  <div class="modal-dialog modal-dialog-scrollable">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="modalLabel">Dados dda dica de saúde</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body" id="modal-dados">
        ...
        <div class="col border rounded text-justify" id="descricaoDica"></div>
        <p class="font-weight-bolder"><a href="#" class="text-decoration-none" id="linkDica" target="_blank">Saiba Mais</a></p>
