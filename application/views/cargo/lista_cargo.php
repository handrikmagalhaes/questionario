...
$registros_por_pagina = 10;
                    ...
                    <input type="text" id="texto_busca" class="form-control float-right" placeholder="Cargo" value="<?php echo $busca; ?>">
                    <div class="input-group-append">
                      <button class="btn btn-default" id="buscar"><i class="fas fa-search pr-2"></i>Buscar</button>
                    </div>
                  </div>
                </div>
			    <?php if($_SESSION['inserir_cargo']){ ?>
				  <a href="{url_base}cargo/cadastro" class="btn btn-primary float-right" id="adicionar">Adicionar Novo</a>
				<?php } ?>
              </div>
              <!-- /.card-header -->
              <div class="card-body p-0" id="lista-cargos">
                <table class="table table-striped">
                  <thead>                  
                    <tr>
                      <th id="thtitulo" onclick="ordena('TITULO_CARGO')" onmouseover="this.style.cursor='pointer';">Título</th>
                  ...
                  <tbody>
                    <?php foreach ($dados['cargos'] as $cargo): ?>
                      <tr>
                        <td onmouseover="this.style.cursor='pointer';"><?php echo $cargo->TITULO_CARGO; ?></td>
                        <td>
                          <?php if ($_SESSION['editar_cargo']): ?> <!-- VERIFICAR PERMISSÃO -->
                            <form method="post" action="{url_base}cargo/edicao" class="btn btn-sm border-0 p-0 m-0">
                          ...
                          <?php endif; ?>
                          <?php if ($_SESSION['excluir_cargo']): ?> <!-- VERIFICAR PERMISSÃO -->
                            <button data-id="<?php echo $cargo->ID_CARGO; ?>" class="btn btn-sm btn-outline-danger btn-excluir border-0 pt-0 pb-0" data-toggle="tooltip" data-placement="top" title="Excluir" data-original-title="Excluir"><i class="fas fa-trash"></i></button>
                          <?php endif; ?>
                        </td>
                      </tr>
                    <?php endforeach; ?>
  ...
  <div class="modal-dialog modal-dialog-scrollable">
    <div class="modal-content">
      ...
      <div class="modal-body" id="modal-dados">
        <p class="font-weight-bold mb-0">Descrição do Cargo</p>
        ...
        <p class="font-weight-bold mb-0">Nível do Cargo</p>
        <p class="font-weight-normal" id="nivelCargo"></p>
