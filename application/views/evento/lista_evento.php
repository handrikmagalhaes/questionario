...
$registros_por_pagina = 10;
			    ...
			    <?php if($_SESSION['inserir_evento']){ ?>
				  <a href="{url_base}evento/cadastro" class="btn btn-primary float-right" id="adicionar">Adicionar Novo</a>
				<?php } ?>
              </div>
              <!-- /.card-header -->
              <div class="card-body p-0" id="lista-eventos">
                <table class="table table-striped">
                  <thead>                  
                    <tr>
                      <th onclick="ordena('TITULO_EVENTO')" onmouseover="this.style.cursor='pointer';">Título</th>
          					  <th onclick="ordena('DT_EVENTO')" onmouseover="this.style.cursor='pointer';">Data</th>
                  ...
                  <tbody>
                    <?php foreach ($dados['eventos'] as $evento): ?>
                      <tr>
                        <td <?php if ($_SESSION['visualizar_link']): ?>onclick="abreModal(<?php echo $evento->ID_EVENTO; ?>)"<?php endif ?> onmouseover="this.style.cursor='pointer';"><?php echo $evento->TITULO_EVENTO; ?></td>
              					<td <?php if ($_SESSION['visualizar_link']): ?>onclick="abreModal(<?php echo $evento->ID_EVENTO; ?>)"<?php endif ?> onmouseover="this.style.cursor='pointer';"><?php echo date("d/m/Y", strtotime($evento->DT_EVENTO)) ?></td>
                        <td>
                          <?php //if ($_SESSION['visualizar_evento']): ?> <!-- VERIFICAR PERMISSÃO -->
                          ...
                          <?php //endif; ?>
                          <?php if ($_SESSION['editar_evento']): ?> <!-- VERIFICAR PERMISSÃO -->
                            <form method="post" action="/evento/edicao" class="btn btn-sm border-0 p-0 m-0">
                              <input type="hidden" name="id" value="<?php echo $evento->ID_EVENTO; ?>">
                          ...
                          <?php endif; ?>
                          <?php if ($_SESSION['excluir_evento']): ?> <!-- VERIFICAR PERMISSÃO -->
                            <button data-id="<?php echo $evento->ID_EVENTO; ?>" class="btn btn-sm btn-outline-danger btn-excluir border-0 pt-0 pb-0" data-toggle="tooltip" data-placement="top" title="Excluir" data-original-title="Excluir"><i class="fas fa-trash"></i></button>
                          <?php endif; ?>
                        </td>
                      </tr>
                    <?php endforeach; ?>
  ...
  <div class="modal-dialog modal-dialog-scrollable">
    <div class="modal-content">
      ...
      <div class="modal-body" id="modal-dados">
        ...
        <div class="col rounded text-justify" id="descricaoEvento"></div>
        <p class="font-weight-bold mb-0">Data</p>
        <span class="font-weight-normal" id="dataEvento">Ativo</span>
