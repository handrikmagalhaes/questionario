...
$registros_por_pagina = 10;
			    ...
			    <?php if($_SESSION['inserir_departamento']){ ?>
				  <a href="{url_base}departamento/cadastro" class="btn btn-primary float-right" id="adicionar">Adicionar Novo</a>
				<?php } ?>
              </div>
              <!-- /.card-header -->
              <div class="card-body p-0" id="lista-departamentos">
                <table class="table table-striped">
                  <thead>                  
                    <tr>
                      <th onclick="ordena('TITULO_DEPARTAMENTO')" onmouseover="this.style.cursor='pointer';">Título</th>
                      <th onclick="ordena('DESCRICAO_DEPARTAMENTO')" onmouseover="this.style.cursor='pointer';">Descrição</th>
                      <th onclick="ordena('DEPARTAMENTO_CHEFIA')" onmouseover="this.style.cursor='pointer';">Subordinado a(o)</th>
                  ...
                  <tbody>
                    <?php foreach ($dados['departamentos'] as $departamento): ?>
                      <tr>
                        <td <?php if ($_SESSION['visualizar_departamento']): ?>onclick="abreModal(<?php echo $departamento->ID_DEPARTAMENTO; ?>)"<?php endif; ?> onmouseover="this.style.cursor='pointer';"><?php echo $departamento->TITULO_DEPARTAMENTO; ?></td>
                        <td <?php if ($_SESSION['visualizar_departamento']): ?>onclick="abreModal(<?php echo $departamento->ID_DEPARTAMENTO; ?>)"<?php endif; ?> onmouseover="this.style.cursor='pointer';"><?php echo $departamento->DESCRICAO_DEPARTAMENTO; ?></td>
                        <td <?php if ($_SESSION['visualizar_departamento']): ?>onclick="abreModal(<?php echo $departamento->ID_DEPARTAMENTO; ?>)"<?php endif; ?> onmouseover="this.style.cursor='pointer';"><?php echo $departamento->DEPARTAMENTO_CHEFIA; ?></td>
                        <td>
                          <?php if ($_SESSION['editar_departamento']): ?> <!-- VERIFICAR PERMISSÃO -->
                            <form method="post" action="/departamento/edicao" class="btn btn-sm border-0 p-0 m-0">
                          ...
                          <?php endif; ?>
                          <?php if ($_SESSION['excluir_departamento']): ?> <!-- VERIFICAR PERMISSÃO -->
                            <button data-id="<?php echo $departamento->ID_DEPARTAMENTO; ?>" class="btn btn-sm btn-outline-danger btn-excluir border-0 pt-0 pb-0" data-toggle="tooltip" data-placement="top" title="Excluir" data-original-title="Excluir"><i class="fas fa-trash"></i></button>
                          <?php endif; ?>
                        </td>
                      </tr>
                    <?php endforeach; ?>
  ...
  <div class="modal-dialog modal-dialog-scrollable">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="modalLabel">Dados do Cargo</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body" id="modal-dados">
        <p class="font-weight-bold mb-0">Descrição do Departamento</p>
        <p class="font-weight-normal" id="descricaoCargo">Teste de advogado</p>
        <p class="font-weight-bold mb-0">Situação</p>
        <p class="font-weight-normal" id="situacaoCargo">Ativo</p>
        <p class="font-weight-bold mb-0">Subordinado ao/à:</p>
        <p class="font-weight-normal" id="subordinadoDepartamento"></p>
        <p class="font-weight-bold mb-0">Colaboradores:</p>
        <div class="col" id="divColaboradores">
        </div>
      </div>
      <div class="modal-footer">
      </div>
