...
class Nivel_acesso extends CI_Controller {
	...
	public function index()
	{
		...
		if(isset($_SESSION['logado'])){
			header('Location: '.base_url().'nivel_acesso/lista');
		} else {
	...
	public function lista()
	{
		...
		if(isset($_SESSION['logado'])){
			...
			set_tema('js', load_js('dist/js/pages/nivel_acesso'), FALSE);
			// DEFINE TITULO
			set_tema('titulo', 'Níveis de Acesso');
			// DEFINE URL_BASE
			set_tema('url_base', base_url());
			// RETORNA DADOS DO REGISTRO
			$this->load->model('NivelAcessoModel');
			$nivel_acesso = new NivelAcessoModel;
			$retorno = $nivel_acesso->listar_niveis_acesso();
			set_tema('dados', $retorno);
	  	    // DEFINE AS PARTES DO TEMPLATE
			set_tema('header', 'header');
			set_tema('template', 'nivel_acesso/lista_nivel_acesso');
	...
	public function visualiza(){
		...
		if(isset($_SESSION['logado'])){
			if(isset($_GET['id'])){
				...
				$retorno = $nivelAcesso->listar_dados_nivel_acesso($_GET['id']);
...
/*		if(!isset($_SESSION)){ 
		...
		if(isset($_SESSION['logado'])){
			if(isset($_POST['id'])){
				...
				$this->load->model('NivelAcessoModel');
				$nivel_acesso = new NivelAcessoModel;
				$retorno = $nivel_acesso->listar_dados_nivel_acesso($_POST['id']);
				set_tema('dados', $retorno);
		  	    // DEFINE AS PARTES DO TEMPLATE
				set_tema('header', 'header');
				set_tema('template', 'nivel_acesso/visualiza_nivel_acesso');
				set_tema('footer', 'footer');
			    // CARREGA TEMPLATE
				load_template();
			} else {
	...
	public function cadastro()
	{
		...
		if(isset($_SESSION['logado'])){
			...
			set_tema('js', load_js('dist/js/pages/nivel_acesso'), FALSE);
			// DEFINE TITULO
			set_tema('titulo', 'Nível Acesso');
			// DEFINE URL_BASE
			set_tema('url_base', base_url());
	  	    // DEFINE AS PARTES DO TEMPLATE
			set_tema('header', 'header');
			set_tema('template', 'nivel_acesso/cadastro_nivel_acesso');
			set_tema('footer', 'footer');
		    // CARREGA TEMPLATE
			load_template();
		} else {
	...
	function cadastrar(){
		...
		if(isset($_SESSION['logado']) ){//&& $_SESSION['inserir_nivel_acesso']){
			$this->load->model('NivelAcessoModel');
			$nivel_acesso = new NivelAcessoModel;
			$retorno = $nivel_acesso->cadastrar_nivel_acesso($_POST['titulo'], $_POST['descricao'], $_POST['situacao'], $_POST['permissoes']);
			echo json_encode($retorno);
		} else {
			header('Location: '.base_url().'login');
		}
	...
	function editar(){
		...
		if(isset($_SESSION['logado']) ){//&& $_SESSION['editar_nivel_acesso']){
			$this->load->model('NivelAcessoModel');
			$nivel_acesso = new NivelAcessoModel;
			$retorno = $nivel_acesso->editar_nivel_acesso($_POST['id'], $_POST['titulo'], $_POST['descricao'], $_POST['situacao'], $_POST['permissoes']);
			echo json_encode($retorno);
		} else {
			header('Location: '.base_url().'login');
		}
	...
	function excluir(){
		...
		if(isset($_SESSION['logado']) ){// && $_SESSION['excluir_nivel_acesso']){
			$this->load->model('NivelAcessoModel');
			$nivel_acesso = new NivelAcessoModel;
			$retorno = $nivel_acesso->excluir_nivel_acesso($_POST['id']);
			echo json_encode($retorno);
		} else {
			header('Location: '.base_url().'login');
		}
