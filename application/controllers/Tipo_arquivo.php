...
class Tipo_arquivo extends CI_Controller {
	...
	public function index()
	{
		...
		if(isset($_SESSION['logado'])){
			header('Location: '.base_url().'tipo_arquivo/lista');
		} else {
	...
	public function lista()
	{
		...
		if(isset($_SESSION['logado'])){
			...
			set_tema('js', load_js('dist/js/pages/tipo_arquivo'), FALSE);
			// DEFINE TITULO
			set_tema('titulo', 'Tipos de Arquivo');
			// DEFINE URL_BASE
			set_tema('url_base', base_url());
			// RETORNA DADOS DO REGISTRO
			$this->load->model('TipoArquivoModel');
			$tipo_arquivo = new TipoArquivoModel;
			$retorno = $tipo_arquivo->listar_tipos_arquivo();
			set_tema('dados', $retorno);
	  	    // DEFINE AS PARTES DO TEMPLATE
			set_tema('header', 'header');
			set_tema('template', 'tipo_arquivo/lista_tipo_arquivo');
	...
	public function visualiza()
	{
		...
		if(isset($_SESSION['logado'])){
			if(isset($_GET['id'])){
				...
				// RETORNA DADOS DO REGISTRO
				$this->load->model('TipoArquivoModel');
				$tipoArquivo = new TipoArquivoModel;
				$retorno = $tipoArquivo->listar_dados_tipo_arquivo($_GET['id']);
		...
		if(isset($_SESSION['logado'])){
			if(isset($_POST['id'])){
				...
				$this->load->model('TipoArquivoModel');
				$tipo_arquivo = new TipoArquivoModel;
				$retorno['dados_tipo_arquivo'] = $tipo_arquivo->listar_dados_tipo_arquivo($_POST['id']);
				// // RETORNA USUÁRIOS
				// $this->load->model('UsuarioModel');
				// $usuario = new UsuarioModel;
				// $retorno['usuarios'] = $usuario->listar_usuarios_tipo_arquivo($_POST['id']);
				// // RETORNA DEPARTAMENTOS
				// $this->load->model('DepartamentoModel');
				// $departamento = new DepartamentoModel;
				// $retorno['departamentos'] = $departamento->listar_departamentos_tipo_arquivo();
				set_tema('dados', $retorno);
		  	    // DEFINE AS PARTES DO TEMPLATE
				set_tema('header', 'header');
				set_tema('template', 'tipo_arquivo/visualiza_tipo_arquivo');
				set_tema('footer', 'footer');
			    // CARREGA TEMPLATE
				load_template();
			} else {
	...
	public function cadastro()
	{
		...
		if(isset($_SESSION['logado'])){
			...
			set_tema('js', load_js('dist/js/pages/tipo_arquivo'), FALSE);
			// DEFINE TITULO
			set_tema('titulo', 'Tipo de Arquivo');
			// DEFINE URL_BASE
			set_tema('url_base', base_url());
	  	    // DEFINE AS PARTES DO TEMPLATE
			set_tema('header', 'header');
			set_tema('template', 'tipo_arquivo/cadastro_tipo_arquivo');
			set_tema('footer', 'footer');
		    // CARREGA TEMPLATE
			load_template();
		} else {
	...
	public function edicao()
	{
		...
		if(isset($_SESSION['logado'])){
			if(isset($_POST['id'])){
				...
				$this->load->model('TipoArquivoModel');
				$tipo_arquivo = new TipoArquivoModel;
				$retorno = $tipo_arquivo->listar_dados_tipo_arquivo($_POST['id']);
				set_tema('dados', $retorno);
		  	    // DEFINE AS PARTES DO TEMPLATE
				set_tema('header', 'header');
				set_tema('template', 'tipo_arquivo/cadastro_tipo_arquivo');
	...
	function cadastrar(){
		...
		if(isset($_SESSION['logado']) ){//&& $_SESSION['inserir_tipo_arquivo']){
			$this->load->model('TipoArquivoModel');
			$tipo_arquivo = new TipoArquivoModel;
			$retorno = $tipo_arquivo->cadastrar_tipo_arquivo($_POST['titulo'], $_POST['descricao'], $_POST['situacao']);
			echo json_encode($retorno);
		} else {
			header('Location: '.base_url().'login');
		}
	...
	function editar(){
		...
		if(isset($_SESSION['logado']) ){//&& $_SESSION['editar_tipo_arquivo']){
			$this->load->model('TipoArquivoModel');
			$tipo_arquivo = new TipoArquivoModel;
			$retorno = $tipo_arquivo->editar_tipo_arquivo($_POST['id'], $_POST['titulo'], $_POST['descricao'], $_POST['situacao']);
			echo json_encode($retorno);
		} else {
			header('Location: '.base_url().'login');
		}
	...
	function excluir(){
		...
		if(isset($_SESSION['logado']) ){// && $_SESSION['excluir_tipo_arquivo']){
			$this->load->model('TipoArquivoModel');
			$tipo_arquivo = new TipoArquivoModel;
			$retorno = $tipo_arquivo->excluir_tipo_arquivo($_POST['id']);
			echo json_encode($retorno);
		} else {
			header('Location: '.base_url().'login');
		}
