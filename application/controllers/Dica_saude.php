...
class Dica_saude extends CI_Controller {
	...
	public function index()
	{
		...
		if(isset($_SESSION['logado'])){
			header('Location: '.base_url().'dica_saude/mostra');
		} else {
	...
	public function lista()
	{
		...
		if(isset($_SESSION['logado'])){
			...
			set_tema('js', load_js('dist/js/pages/dica_saude'), FALSE);
			// DEFINE TITULO
			set_tema('titulo', 'Dicas de Saúde e Mensagens');
			// DEFINE URL_BASE
			set_tema('url_base', base_url());
			// RETORNA DADOS DO REGISTRO
			$this->load->model('DicaSaudeModel');
			$DicaSaude = new DicaSaudeModel;
			$retorno = $DicaSaude->listar_dicas_saude();
			set_tema('dados', $retorno);
	  	    // DEFINE AS PARTES DO TEMPLATE
			set_tema('header', 'header');
			set_tema('template', 'dica_saude/lista_dica_saude');
	...
	public function visualiza()
	{
		...
		if(isset($_SESSION['logado'])){
			/*if(isset($_POST['id'])){
				...
				$this->load->model('DicaSaudeModel');
				$DicaSaude = new DicaSaudeModel;
				$retorno = $DicaSaude->listar_dados_dica_saude($_POST['id']);
				set_tema('dados', $retorno);
		  	    // DEFINE AS PARTES DO TEMPLATE
				set_tema('header', 'header');
				set_tema('template', 'dica_saude/visualiza_dica_saude');
				set_tema('footer', 'footer');
			    // CARREGA TEMPLATE
				load_template();*/
				if(isset($_GET['id'])){
				  ...
				  $this->load->model('DicaSaudeModel');
				  $DicaSaude = new DicaSaudeModel;
				  $retorno = $DicaSaude->listar_dados_dica_saude($_GET['id']);
	...
	public function cadastro()
	{
		...
		if(isset($_SESSION['logado'])){
			...
			set_tema('js', load_js('dist/js/pages/dica_saude'), FALSE);
			// DEFINE TITULO
			set_tema('titulo', 'Dica de Saúde');
			// DEFINE URL_BASE
			set_tema('url_base', base_url());
	  	    // DEFINE AS PARTES DO TEMPLATE
			set_tema('header', 'header');
			set_tema('template', 'dica_saude/cadastro_dica_saude');
			set_tema('footer', 'footer');
		    // CARREGA TEMPLATE
			load_template();
		} else {
	...
	function cadastrar(){
		...
		if(isset($_SESSION['logado']) ){//&& $_SESSION['inserir_dica_saude']){
			$this->load->model('DicaSaudeModel');
			$DicaSaude = new DicaSaudeModel;
			$retorno = $DicaSaude->cadastrar_dica_saude($_POST['titulo'], $_POST['link'], $_POST['descricao'], $_POST['situacao']);
			echo json_encode($retorno);
		} else {
			header('Location: '.base_url().'login');
		}
	...
	function editar(){
		...
		if(isset($_SESSION['logado']) ){//&& $_SESSION['editar_dica_saude']){
			$this->load->model('DicaSaudeModel');
			$DicaSaude = new DicaSaudeModel;
			$retorno = $DicaSaude->editar_dica_saude($_POST['id'], $_POST['titulo'], $_POST['link'], $_POST['descricao'], $_POST['situacao']);
			echo json_encode($retorno);
		} else {
			header('Location: '.base_url().'login');
		}
	...
	function excluir(){
		...
		if(isset($_SESSION['logado']) ){// && $_SESSION['excluir_dica_saude']){
			$this->load->model('DicaSaudeModel');
			$DicaSaude = new DicaSaudeModel;
			$retorno = $DicaSaude->excluir_dica_saude($_POST['id']);
			echo json_encode($retorno);
		} else {
			header('Location: '.base_url().'login');
		}
