...
class Loas extends CI_Controller {
	...
	public function index()
	{
		...
		if(isset($_SESSION['logado'])){
			header('Location: '.base_url().'loas/lista');
		} else {
	...
	public function lista()
	{
		...
		if(isset($_SESSION['logado'])){
			...
			set_tema('js', load_js('dist/js/pages/loas'), FALSE);
			// DEFINE URL_BASE
			...
			set_tema('template', 'loas/lista_loas');
	...
	public function cadastro()
	{
		...
		if(isset($_SESSION['logado'])){
			...
			// CARREGA TEMPLATE
	  	    // DEFINE AS PARTES DO TEMPLATE
			set_tema('header', 'header');
			set_tema('template', 'loas/cadastro_loas');
			set_tema('footer', 'footer');
		    // CARREGA TEMPLATE
			load_template();
		} else {
	...
	public function edicao()
	{
		...
		if(isset($_SESSION['logado'])){
			...
			$this->load->model('LoasModel');
			$loas = new LoasModel;
			$retorno = $loas->buscar($_GET['id']);
	...
	function cadastrar(){
		...
		if(isset($_SESSION['logado']) ){//&& $_SESSION['inserir_usuario']){
			...
			if ($dados['periciando_id'] == 0){
				...
				$this->load->model('LoasModel');
				$loas = new LoasModel;
				$retorno = $loas->cadastrar_loas($dados);
				if ($retorno){
			...
			header('Location: '.base_url().'loas/login');
		}
	}
	...
	function alterar(){
		...
		if(isset($_SESSION['logado'])){
			...
			$this->load->model('LoasModel');
			$loas = new LoasModel;
			$retorno = $loas->alterar_loas($dados);
			if ($retorno){
