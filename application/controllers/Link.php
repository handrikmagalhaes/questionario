...
class Link extends CI_Controller {
	...
	public function index()
	{
		...
		if(isset($_SESSION['logado'])){
			header('Location: '.base_url().'link/lista');
		} else {
	...
	public function visualiza()
	{
		...
		if(isset($_SESSION['logado'])){
			/*if(isset($_POST['id'])){
				...
				$this->load->model('LinkModel');
				$link = new LinkModel;
				$retorno = $link->listar_dados_link($_POST['id']);
				set_tema('dados', $retorno);
		  	    // DEFINE AS PARTES DO TEMPLATE
				set_tema('header', 'header');
				set_tema('template', 'link/visualiza_link');
				set_tema('footer', 'footer');
			    // CARREGA TEMPLATE
				load_template();
			} else {*/
			if(isset($_GET['id'])){	
				...
				$this->load->model('LinkModel');
				$link = new LinkModel;
				$retorno = $link->listar_dados_link($_GET['id']);
	...
	public function cadastro()
	{
		...
		if(isset($_SESSION['logado'])){
			...
			set_tema('js', load_js('dist/js/pages/link'), FALSE);
			// DEFINE TITULO
			set_tema('titulo', 'Link');
			// DEFINE URL_BASE
			set_tema('url_base', base_url());
	  	    // DEFINE AS PARTES DO TEMPLATE
			set_tema('header', 'header');
			set_tema('template', 'link/cadastro_link');
			set_tema('footer', 'footer');
		    // CARREGA TEMPLATE
			load_template();
		} else {
	...
	function cadastrar(){
		...
		if(isset($_SESSION['logado']) ){//&& $_SESSION['inserir_link']){
			$this->load->model('LinkModel');
			$link = new LinkModel;
			$retorno = $link->cadastrar_link($_POST['titulo'], $_POST['caminho'], $_POST['tipo'], $_POST['descricao'], $_POST['situacao']);
			echo json_encode($retorno);
		} else {
			header('Location: '.base_url().'login');
		}
	...
	function editar(){
		...
		if(isset($_SESSION['logado']) ){//&& $_SESSION['editar_link']){
			$this->load->model('LinkModel');
			$link = new LinkModel;
			$retorno = $link->editar_link($_POST['id'], $_POST['titulo'], $_POST['caminho'], $_POST['tipo'], $_POST['descricao'], $_POST['situacao']);
			echo json_encode($retorno);
		} else {
			header('Location: '.base_url().'login');
		}
	...
	function excluir(){
		...
		if(isset($_SESSION['logado']) ){// && $_SESSION['excluir_link']){
			$this->load->model('LinkModel');
			$link = new LinkModel;
			$retorno = $link->excluir_link($_POST['id']);
			echo json_encode($retorno);
		} else {
			header('Location: '.base_url().'login');
		}
